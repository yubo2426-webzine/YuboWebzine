...
const StatsReport: React.FC<{ centers: Record<number, string> }> = ({ centers }) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data } = await supabase.from('care_applications').select('*')
  ...
  const byKey = (fn: (r: AppRow) => string) => {
    const m: Record<string, number> = {};
    rows.forEach(r => { const k = fn(r); m[k] = (m[k] || 0) + 1; });
    return Object.entries(m).sort((a, b) => b[1] - a[1]);
  };
  const byCenter = byKey(r => centers[r.center_id] || `기관 ${r.center_id}`);
  const byType = byKey(r => r.care_type);
  const byStatus = byKey(r => r.status);

  const downloadReport = () => {
    const lines: string[] = [];
    lines.push(`함께누리웹진 돌봄 신청 월간 통계,${month}`);
    lines.push(`총 신청,${total}건`);
    lines.push('');
    lines.push('구분,항목,건수,비율');
    byCenter.forEach(([k, n]) => lines.push(`기관별,"${k}",${n},${pct(n)}`));
    byType.forEach(([k, n]) => lines.push(`시간대별,"${k}",${n},${pct(n)}`));
    byStatus.forEach(([k, n]) => lines.push(`상태별,"${k}",${n},${pct(n)}`));
    const blob = new Blob(['\uFEFF' + lines.join('\n')], { type: 'text/csv;charset=utf-8' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `돌봄신청_월간통계_${month}.csv`;
    a.click();
    URL.revokeObjectURL(a.href);
  };

  const Section: React.FC<{ title: string; data: [string, number][] }> = ({ title, data }) => (
    <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl p-5">
      <h4 className="font-black text-slate-800 dark:text-white mb-3">{title}</h4>
      {data.length === 0 ? <p className="text-sm font-bold text-slate-400">데이터 없음</p> : (
        <div className="space-y-2">
          {data.map(([k, n]) => (
            <div key={k} className="flex items-center gap-3">
              <span className="text-sm font-bold text-slate-600 dark:text-slate-300 w-40 shrink-0 truncate">{k}</span>
              <div className="flex-1 h-3 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
                <div className="h-full bg-sky-400 rounded-full" style={{ width: pct(n) }}/>
              </div>
              <span className="text-xs font-black text-slate-500 w-20 text-right shrink-0">{n}건 ({pct(n)})</span>
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-[2rem] border border-slate-100 dark:border-slate-700 p-7 md:p-9">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <h3 className="text-xl font-black text-slate-800 dark:text-white">월간 신청 통계</h3>
        <div className="flex items-center gap-2">
          <input type="month" value={month} onChange={e => setMonth(e.target.value)}
            className="h-11 px-4 rounded-full border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900 font-black text-sm text-slate-700 dark:text-slate-200"/>
          <button onClick={downloadReport}
            className="h-11 px-5 rounded-full bg-emerald-500 text-white font-black text-sm flex items-center gap-2 hover:bg-emerald-600">
            <Download size={14}/> 보고서 CSV
          </button>
        </div>
      </div>
      {loading ? <div className="flex justify-center py-16"><Loader2 className="animate-spin text-sky-500" size={32}/></div> : (
        <>
          <p className="font-black text-slate-700 dark:text-slate-200 mb-5">{month} 총 신청 <span className="text-sky-500 text-xl">{total}</span>건</p>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <Section title="기관별" data={byCenter}/>
            <Section title="시간대별" data={byType}/>
            <Section title="상태별" data={byStatus}/>
          </div>
        </>
      )}
    </div>
