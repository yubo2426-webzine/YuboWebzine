...
const SIDE_TABS = [
  ...
  { id: 'news',       label: '뉴스',     icon: Newspaper,              color: 'text-rose-500' },
  ...
  return (
    <div className="max-w-[1400px] mx-auto w-full flex flex-col gap-5">
      ...
      <div className="flex flex-col lg:flex-row gap-5 items-start">
        ...
        <nav className="w-full lg:w-[340px] shrink-0 flex flex-col gap-2.5">
          {TOPICS.map(t => {
            const Icon = t.icon;
            return (
              <button
                ...
                className="group w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl
                  px-4 py-3.5 lg:px-5
                  flex flex-row items-center gap-3.5 text-left
              ...
              >
                <span className={`w-11 h-11 rounded-xl ${t.accent} flex items-center justify-center shrink-0`}>
                  <Icon size={22} className={t.iconColor} />
                </span>

                <span className="flex-1 min-w-0 flex flex-col">
                  <span className="font-black text-base lg:text-lg text-slate-800 dark:text-white leading-tight">
                    {t.sub}
                  </span>
                  <span className="font-bold text-xs lg:text-sm text-slate-500 dark:text-slate-400 leading-snug break-keep mt-0.5">
                    {t.index}
                  </span>
                </span>
        ...
        {SIDE_TABS.map(t => {
          ...
          return (
            <button
              ...
              className="bg-white dark:bg-slate-800 border border-amber-200 dark:border-amber-900 rounded-xl
                px-2 py-3
                flex flex-col items-center justify-center gap-1.5
                font-black text-slate-800 dark:text-slate-100 text-center
                hover:border-amber-400 dark:hover:border-amber-700 hover:bg-amber-50 dark:hover:bg-amber-950/20
                transition-colors duration-200 group"
            >
              <Icon size={20} className={`${t.color} group-hover:scale-110 transition-transform`} />
              <span className="text-xs md:text-sm leading-tight break-keep">{t.label}</span>
            </button>
          );
        })}
      </div>
