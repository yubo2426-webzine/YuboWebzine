...
const LoginForm: React.FC = () => {
  ...
  const loginWithGoogle = async () => {
    ...
    setBusy(false);
    if (err) setError('구글 로그인에 실패했습니다. 나중에 다시 시도해 주세요.');
  };
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-20">
      <div className="bg-white dark:bg-slate-800 rounded-[2rem] border border-slate-100 dark:border-slate-700 p-9 shadow-lg">
        ...
        <p className="text-sm font-bold text-slate-500 dark:text-slate-400 mb-7">구글 계정으로 로그인하세요.<br/>돌봄 신청 내역과 문의 관리는 승인된 계정만 이용할 수 있습니다.</p>
        
        {error && <p className="mb-4 text-sm font-black text-rose-500 bg-rose-50 dark:bg-rose-900/20 rounded-lg px-3 py-2">{error}</p>}
        
        <button onClick={loginWithGoogle} disabled={busy}
          className="w-full h-13 py-3.5 rounded-2xl bg-white dark:bg-slate-700 border-2 border-slate-200 dark:border-slate-600 text-slate-800 dark:text-white font-black flex items-center justify-center gap-3 hover:bg-slate-50 dark:hover:bg-slate-600 disabled:opacity-60 transition-all shadow-md">
          {busy ? (
            <><Loader2 className="animate-spin" size={18}/> 로그인 중...</>
          ) : (
            <>
