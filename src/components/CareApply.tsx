...
const CareApply: React.FC = () => {
  ...
  const [applyPeriod, setApplyPeriod] = useState<{ start: string; end: string }>({ start: '', end: '' });
  const [reapply, setReapply] = useState<any>(null);   // 재신청 시 미리 채울 값
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: st } = await supabase.from('app_settings').select('key, value')
        .in('key', ['apply_start', 'apply_end']);
      if (st) {
        const m = Object.fromEntries(st.map((r: any) => [r.key, r.value]));
        setApplyPeriod({ start: m.apply_start || '', end: m.apply_end || '' });
      }
      setLoading(false);
    })();
  }, []);
    ...
    <div className="max-w-7xl mx-auto px-4 py-10">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
        ...
        {(() => {
          ...
          const closed = (applyPeriod.start && today < applyPeriod.start) || (applyPeriod.end && today > applyPeriod.end);
          if (!closed) return null;
          return (
            <div className="mb-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-2xl px-5 py-4 font-black text-amber-700 dark:text-amber-300 text-sm">
              ⏰ 지금은 신청 기간이 아닙니다.
              {applyPeriod.start && applyPeriod.end && <> (신청 기간: {applyPeriod.start} ~ {applyPeriod.end})</>}
              {' '}기관 정보 열람은 가능합니다.
            </div>
          );
        })()}
      ...
      {selected && showForm && (
        <ApplyForm center={selected} initial={reapply}
          onClose={() => { setShowForm(false); setSelected(null); setReapply(null); }}/>
      )}
      {showLookup && <LookupModal onClose={() => setShowLookup(false)}
        onReapply={(r) => {
          const c = centers.find(x => x.id === r.center_id);
          if (!c) { alert('해당 기관을 찾을 수 없습니다.'); return; }
          setShowLookup(false);
          setReapply(r);
          setSelected(c);
          setShowForm(true);
...
// 신청 폼
// ─────────────────────────────────────────────
const ApplyForm: React.FC<{ center: any; onClose: () => void; initial?: any }> = ({ center, onClose, initial }) => {
  const [f, setF] = useState({
    guardian_name: initial?.guardian_name || '', guardian_phone: '',
    child_names: initial?.child_names || '', child_count: initial?.child_count || 1,
    care_type: (initial?.care_type || '오전') as string, use_date: '', use_time: '', memo: '',
  });
  const [hp, setHp] = useState('');                          // 봇 방지 허니팟 (사람 눈엔 안 보임)
...
// 접수번호로 내 신청 조회
// ─────────────────────────────────────────────
const LookupModal: React.FC<{ onClose: () => void; onReapply: (r: any) => void }> = ({ onClose, onReapply }) => {
  ...
  const lookup = async () => {
    ...
    if (!supabase) return setError('서버 연결에 실패했습니다.');
    setBusy(true);
    const { data, error: err } = await supabase.rpc('get_my_applications', {
  ...
  const cancelApp = async (r: any) => {
    if (!supabase) return;
    if (!window.confirm(`${r.use_date} ${r.care_type} 돌봄 신청을 취소할까요?\n취소 후에는 되돌릴 수 없으며, 필요하면 다시 신청해야 합니다.`)) return;
    const { data: ok } = await supabase.rpc('cancel_my_application', {
      p_receipt_no: r.receipt_no, p_phone: phone.trim(),
    });
    if (ok) {
      setResult(prev => (prev || []).map(x => x.receipt_no === r.receipt_no ? { ...x, status: '취소' } : x));
    } else {
      setError('취소할 수 없습니다. 이미 처리된 신청은 기관에 직접 문의해 주세요.');
    }
  };
  ...
  return (
    <Modal onClose={onClose}>
      ...
      {result && result.map((r, i) => (
        <div key={i} className="mt-4 bg-slate-50 dark:bg-slate-900 rounded-2xl p-5 space-y-2 font-bold text-sm text-slate-600 dark:text-slate-300">
          <div className="flex items-center justify-between gap-2 flex-wrap">
            <span className="font-black text-slate-800 dark:text-white">{r.center_name}</span>
            <span className="flex items-center gap-2">
              <span className="text-[11px] text-slate-400">{r.receipt_no}</span>
          ...
          <div className="flex gap-2 pt-1">
            {r.status === '접수' && (
              <button onClick={() => cancelApp(r)}
                className="h-9 px-4 rounded-full bg-rose-500 text-white font-black text-xs hover:bg-rose-600">신청 취소</button>
            )}
            <button onClick={() => onReapply(r)}
              className="h-9 px-4 rounded-full bg-sky-500 text-white font-black text-xs hover:bg-sky-600">같은 조건으로 재신청</button>
          </div>
        </div>
      ))}
